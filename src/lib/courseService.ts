...
export async function fetchMyCourses(): Promise<SavedCourse[]> {
  ...
  const { data, error } = await supabase
    ...
    .eq('user_id', user.id)
    .order('created_at', { ascending: false })
  if (error) { console.error('[courseService] fetchMyCourses 실패:', error.message); return [] }
  return (data ?? []).map(rowToSavedCourse)
}
...
export async function insertMyCourse(course: SavedCourse): Promise<boolean> {
  ...
  if (error) { console.error('[courseService] insertMyCourse 실패:', error.message); return false }
  return true
}
...
export async function updateMyCourse(id: string, partial: Partial<SavedCourse>): Promise<boolean> {
  ...
  const { error } = await supabase.from('user_courses').update(payload).eq('id', id)
  if (error) { console.error('[courseService] updateMyCourse 실패:', error.message); return false }
  return true
}
